...
1. **[P1, Security] Add a total query-buffer cap. DONE (#528, #527).** read_buf no longer grows unbounded pre-auth. The serve loop enforces a runtime-settable `query-buffer-limit` (default 1 GB, the Redis `client-query-buffer-limit` analog) after every recv on BOTH datapaths (serve.rs tokio and io_uring), closing the connection the moment the accumulated inbound buffer crosses the cap, so a `*1048576\r\n` header plus dribbled elements is bounded, not multi-GB. Independently, a single bulk-string length header over `proto-max-bulk-len` (default 512 MB, Redis-compatible) is a hard `invalid bulk length` protocol error decided from the header alone, so the claimed payload is rejected before it is ever reserved or buffered (reject-before-alloc). Both are boot-config plus `CONFIG SET`/`CONFIG GET` knobs; #527 also made boot reject `proto-max-bulk-len 0` (a zero ceiling would reject every value), matching the runtime setter. Verified by the slow-dribble multibulk disconnect tests (conn_limits.rs), the header-rejects-before-payload decode test (decode.rs), and the boot-validation test (config lib.rs); HARDENING.md now documents the real per-connection bound.
...
20. **[P2, Security] Fix TLS docs or ship the features. DONE (#561).** The stale `docs/design/TLS.md` promised restart-free cert reload and client-port mTLS; the acceptor is built once at boot (serve.rs) with `.with_no_client_auth()` (tls.rs). The docs are now reconciled: `docs/design/TLS.md` carries a "Shipped status" section stating rotation needs a restart and client-port mTLS is unavailable, and a new operator guide `docs/TLS.md` documents the covered-vs-uncovered surface (client port, cluster bus + repl, no client mTLS, no hot reload) with a cert quickstart. Hot cert reload is filed as a follow-up (#563).

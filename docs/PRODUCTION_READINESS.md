...
| Security & public trust | 9.0 (7.5) | Query-buffer cap (PR #542), parser fuzzing (PR #541), hot TLS cert reload (PR #584), and the unauthenticated-cluster-bus warning (PR #560) all landed; overflow-checks-off in release and console web hardening (#369) remain |
...
| Troubleshooting & operability | 8.5 (6.5) | Node-wide INFO, symbolized crashes (PR #553), an operator RUNBOOK (PR #552), and env-var warnings (PR #560); the MONITOR doc mismatch is the notable residual |
...
| Goal completion audit | 7.5 (n/s) | Issue-resolution largely done and the #517 hop-elimination chain is now complete (#525 merged + driver-matrix cluster leg #569); the competitive goal is unproven pending a version-pinned Dragonfly re-bench (#507 open) |
...
27. **[P3, Monitoring] Real run_id / redis_mode / MONITOR. OPEN.** run_id is still the 40-zero placeholder (observe/lib.rs:2073-2075), redis_mode is hardwired `standalone` (observe/lib.rs:1828), and the MONITOR command is still not implemented while README.md:143 claims secret redaction for it.
...
- **run_id / redis_mode / MONITOR honesty (item 27).** A placeholder run_id and hardwired `standalone` mode mislead exporters, and README claims redaction for a MONITOR command that does not exist.
...
**Verdict: partially met, competitive position unproven pending a pinned re-bench.** The issue-resolution half is largely done (the two "hygiene breaks the goal" claims were refuted), and one of the three gaps from the prior audit has since closed:

1. **The scoreboard is still stale.** README.md:313-350 publishes a higher-core run against Redis 8.8.0 and a Dragonfly image, but a version-pinned Dragonfly re-run is explicitly still a follow-up (README.md:316), so the merged perf levers are unquantified against a fixed competitor and #507 remains OPEN.
2. **The #517 hop-elimination chain is now COMPLETE.** PR #525 merged (N-shard CLUSTER SLOTS projection + MOVED, eliminating the hop for cluster-aware clients) and PR #569 landed the driver-matrix cluster leg proving the hop is eliminated; issue #517 is CLOSED. What remains under #507 is the acceptance wording (which leg defines "within 3%"), not the lever itself.
...
1. Run one pinned re-bench of current main against a version-pinned Dragonfly (docs/bench/COMPETITORS.md), executing BOTH legs: single-endpoint redis-benchmark (the #507 acceptance as written) and memtier --cluster-mode against shard-owners (the #517 acceptance), under the existing AWS teardown rules. One afternoon of instance time.
2. Amend #507 to name which leg defines "within 3%", so done can be declared or the remaining constant-factor work scoped.
...
When these are done, the competitive goal is either met and provable, or the remaining gap is a measured number with named levers, not an unknown.

...
| **KeyDB** (the README also names it) | **v6.3.4** (DORMANT; last release Oct 2023) | 2023-10 | BSD-3-Clause | Tracks the Redis 6 lineage: the classic `dict` (no kvobj / no embedded key), so ~16-24 bytes/item overhead like pre-8.x Redis, plus its multi-thread bookkeeping. Measured heaviest of the RESP set in the head-to-head (240 B/key for 128B values). Boots with `--server-threads`. |
| **Memcached** (non-RESP; memory only) | **1.6.x** (runner apt) | varies | BSD-3-Clause | Slab allocator: items are rounded up to a slab class, so per-item overhead is the item header (~48-56 bytes) plus the slab-class rounding slack. Does NOT speak RESP, so only `stats bytes` (item bytes stored) is comparable; throughput is cross-protocol and out of scope. |

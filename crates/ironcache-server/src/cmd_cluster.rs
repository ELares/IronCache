...
fn cluster_help() -> Value {
    let lines: &[&str] = &[
        ...
        "The following mutate cluster state and require raft or shard-owners mode:",
        "MEET <ip> <port> [<bus-port>]",
        "    Introduce another node into this node's cluster (run on the leader).",
        "FORGET <node-id>",
        "    Remove a node from the cluster.",
        "REPLICATE <node-id> <slot> [<slot> ...]",
        "    Make this node a replica of <node-id> for the named slots (per-slot form).",
        "FAILOVER",
        "    Promote this in-sync replica to owner of the slots it replicates (FORCE/TAKEOVER unsupported).",
        "ADDSLOTS <slot> [<slot> ...]",
        "    Assign the named slots to this node.",
        "ADDSLOTSRANGE <start> <end> [<start> <end> ...]",
        "    Assign the named slot ranges to this node.",
        "DELSLOTS <slot> [<slot> ...]",
        "    Unassign the named slots from this node.",
        "DELSLOTSRANGE <start> <end> [<start> <end> ...]",
        "    Unassign the named slot ranges from this node.",
        "SETSLOT <slot> NODE <id> | MIGRATING <id> | IMPORTING <id> | STABLE",
        "    Set the owner of a slot, or mark it migrating/importing/stable for resharding.",
        "FLUSHSLOTS",
        "    Remove all slot assignments from this node.",
        "REBALANCE APPLY",
        "    Propose the balanced-slot plan as committed migrations (raft mode).",
        "BUMPEPOCH",
        "    Advance this node's config epoch.",
        "SET-CONFIG-EPOCH <epoch>",
        "    Set this node's config epoch.",
        "HELP",
        "    Print this help.",
    ];

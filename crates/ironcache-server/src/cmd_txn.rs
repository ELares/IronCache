...
pub(crate) mod tests {
    ...
    /// 2 INTERNAL cross-shard verbs (`__ICSTORESET` + `__ICSTOREZSET`, COORDINATOR.md #107
    /// Stage 2b -- real dispatch arms + registry entries, but client-unreachable), so 150
    /// dispatch arms total. This asserts a COUNT only (not values -- the value-level cover is
    /// the set-equality in `table_covers_every_dispatch_arm`), so it is NOT a second source of
    /// truth. A drift here flags that a dispatch arm was added or removed; update the registry
    /// + the hand-list.
    #[test]
    fn dispatch_arm_list_has_the_expected_count() {
        assert_eq!(
            ...
            150,
            "the dispatch-arm hand-list drifted from the 148 client commands + 2 internal verbs"
        );
    }

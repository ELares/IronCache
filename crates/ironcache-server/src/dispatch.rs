...
// #625: the connection + server-admin command handlers, relocated verbatim. `dispatch_inner` calls
// the entrypoints; `ShutdownMode` / `parse_shutdown` / `in_sync_replica_count` are re-exported `pub`
// so the crate root's `pub use dispatch::{...}` keeps resolving them.
#[path = "dispatch_conn_cmd.rs"]
mod dispatch_conn_cmd;
pub use dispatch_conn_cmd::{ShutdownMode, in_sync_replica_count, parse_shutdown};
use dispatch_conn_cmd::{
    cmd_auth, cmd_client, cmd_command, cmd_echo, cmd_hello, cmd_lolwut,
    cmd_persist_bgsave_fallback, cmd_persist_lastsave_fallback, cmd_persist_save_fallback,
    cmd_ping, cmd_select, cmd_shutdown_fallback, cmd_wait_fallback,
};
// `constant_time_eq` is referenced only by the dispatch unit tests (dispatch_tests.rs via
// `use super::*`), so its re-export is `#[cfg(test)]`.
#[cfg(test)]
pub(crate) use dispatch_conn_cmd::constant_time_eq;
// #625: WATCH/UNWATCH/EXEC transaction handling; `dispatch_inner` calls the entrypoints.
#[path = "dispatch_txn.rs"]
mod dispatch_txn;
use dispatch_txn::{cmd_unwatch, cmd_watch, exec_transaction};
// #625: the ACL / pre-auth gating. The dispatch engine + the serve router both call these; they are
// re-exported `pub` so the crate root's `pub use dispatch::{...}` keeps resolving them.
#[path = "dispatch_acl.rs"]
mod dispatch_acl;
pub use dispatch_acl::{acl_enforce, acl_resolve_if_stale, command_allowed_pre_auth};

...
pub fn spec_of(cmd_upper: &[u8]) -> Option<&'static CommandSpec> {
    ...
    let spec: &'static CommandSpec = match cmd_upper {
        ...
        // Hash field TTL (#408). Setters (HEXPIRE family + HPERSIST) are writes; the readers
        // (HTTL family) are reads. All take the key at arg1 and a FIELDS block (arity >= 6 for
        // the setters, which carry a ttl arg, >= 5 for the readers + HPERSIST).
        b"HEXPIRE" => &CommandSpec {
            name: b"HEXPIRE",
            arity: Min(6),
            class: KeyedSingle,
            key_spec: Arg1,

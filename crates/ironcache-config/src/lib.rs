...
impl Config {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        // TRANSPORT TLS (#105, docs/design/TLS.md). When the listener is TLS-only (`tls = on`),
        // a cert AND a key path are REQUIRED, and both must be READABLE at boot: a TLS-only
        // listener with no usable cert/key would reject every handshake, so this is a LOUD boot
        // error (the operator sees it immediately) rather than a listener that silently refuses
        // all clients. The actual PEM parse + rustls acceptance happens at boot in the runtime
        // layer; here we only assert the paths are set and the files OPEN (a cheap, clear
        // pre-flight: a typo'd path or an unreadable key fails here with a precise field). When
        // `tls = off` (the default) this block is skipped entirely, so the plaintext path is
        // byte-unchanged and cert/key are ignored (they may be set but are never read).
        if self.tls == TlsMode::On {
            let cert = self
                ...
                .ok_or_else(|| ConfigError::Invalid {
                    field: "tls_cert_path",
                    reason:
                        "is required when tls = on (the TLS listener needs a certificate chain)"
                            .to_owned(),
                })?;
            let key = self
                .tls_key_path
            ...
            // Pre-flight readability: open each file so a bad path / permission problem is a
            // clear boot error here, not an opaque handshake failure later. `field` is &'static
            // str, so the offending path goes in `reason`.
...
impl ConfigOverlay {
    ...
    ///
    /// `too_many_lines` is allowed: this is the FLAT one-`if let` per env var transcription (one
    /// recognized `IRONCACHE_*` variable per block), the same shape as `apply_to` / `spec_of`;
    /// splitting it would scatter the env-var contract across helpers with no readability gain.

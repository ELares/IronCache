...
/// Read and parse the PEM cert CHAIN at `path` into rustls' owned DER certificates.
/// A leaf-first chain with any intermediates is supported (every `CERTIFICATE` block
/// is taken in file order). Errors if the file is unreadable or holds no certificate.
...
/// Read and parse the PEM PRIVATE KEY at `path`, accepting PKCS#8, RSA (PKCS#1), and
/// SEC1 (EC) encodings (`rustls_pemfile::private_key` takes the FIRST key block of any
/// of those forms). Errors if the file is unreadable or holds no private key.
fn load_key(path: &str) -> Result<PrivateKeyDer<'static>, TlsConfigError> {
    ...
    rustls_pemfile::private_key(&mut reader)
        ...
        .ok_or_else(|| TlsConfigError::Pem {
            path: path.to_owned(),
            reason: "no private key found (expected a PKCS#8, RSA, or SEC1 PEM block)".to_owned(),
        })
}

...
1. **Safe scale-out / scale-in**  -- learner-join, online reshard (`REBALANCE APPLY` arms the copies + per-slot `SETSLOT NODE` finalized by polling `COUNTKEYSINSLOT`), and drain-slots-then-`FORGET`-then-scale-then-retain-PVC. This is the biggest chart shortfall (§P1-7) and the classic Bitnami redis-cluster failure mode. **The decisive sharp edge: after any online reshard the runtime slot map diverges from the static ConfigMap (S2).** The Raft log wins on restart (turnkey bootstrap is fresh-only gated, and a conflicting topology refuses to boot rather than overwriting committed membership), so a routine `helm upgrade`/pod-roll does **not** silently revert ownership  -- but the stale ConfigMap becomes a misleading source of truth, a *fresh re-provision* (wiped PVCs/logs) falls back to the pre-reshard layout, and a conflicting topology edit can block boot (see `SCALING.md` §5). A declarative chart *cannot* own a slot map that mutates at runtime; an operator reconciling desired-vs-actual slot ownership is the only correct home for it. This is the strongest single argument for the operator.
